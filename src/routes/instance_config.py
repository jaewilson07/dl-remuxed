...
async def get_is_user_invite_notifications_enabled(
    ...
    if not res.is_success:
        raise ToggleSocialUsers_Error(res=res)
...
class ToggleUserInvite_Error(dmde.RouteError):
    def __init__(
        self, res: rgd.ResponseGetData, message="failure to toggle user invite enabled"
    ):
        super().__init__(res=res, message=message)
...
async def toggle_is_user_invite_enabled(
    ...
    if not res.is_success:
        raise ToggleUserInvite_Error(res=res, message=res.response)
...
async def set_allowlist(
    ...
    try:
        ...
        raise Allowlist_UnableToUpdate(
            res=res,
            message="unable to GET allowlist - either pass FullAuth or return_raw = True",
        ) from e

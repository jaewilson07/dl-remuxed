...
# %% auto 0
__all__ = ['AccessToken_Error', 'get_access_tokens', 'get_access_token_by_id', 'generate_expiration_unixtimestamp',
           'AccessToken_GenerationError', 'generate_access_token', 'AccessToken_RevokeError', 'revoke_access_token']
...
@gd.route_function
async def get_access_token_by_id(auth: dmda.DomoAuth,
                                 access_token_id: int,
                                 session: httpx.AsyncClient = None,
                                 debug_api: bool = False,
    ...
    token = next((token for token in res.response if token['id'] == access_token_id), None)

    if not token:
        raise AccessToken_Error(res=res, 
                                message=f"Access token with ID {access_token_id} not found.")

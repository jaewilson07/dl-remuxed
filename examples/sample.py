...
async def get_unique_datasets(target_auth) -> list[dmds.DomoDataset]:

    datasets: list[dmds.DomoDataset] = await dmdc.DomoDatacenter(
        auth=target_auth
    ).search_datasets()
    ...
    for ds in datasets:
        if (
            ds.data_provider_type not in providers
            and "snowflake" in ds.data_provider_type
        ):
            providers.add(ds.data_provider_type)
            unique_datasets.append(ds)

    await dmce.gather_with_concurrency(
        *[ds.get_streams(debug_api=False) for ds in unique_datasets], 10
    )

    return unique_datasets


async def main(test_instance: str = "playstation-d2c"):
    ...
    unique_datasets = await get_unique_datasets(target_auth=target_auth)

    print(f"\n✅ Completed! Found {len(unique_datasets)} missing account configs")

...
async def test_cell_3(token_auth=token_auth):
    ...
    start_char_index = calculate_character_index_of_line_number(test_fn.lineno-1, full_text = res.response['code'])
    end_char_index = calculate_character_index_of_line_number(test_fn.end_lineno-1, full_text = res.response['code'])
...
async def test_cell_6(token_auth=token_auth):
    ...
    ce_fn = CodeEngineManifest_Function.from_ast_function_def(test_fn, original_module_string=res.response['code'])

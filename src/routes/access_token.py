...
async def get_access_token_by_id(
    ...
    token = next(
        (token for token in res.response if token["id"] == access_token_id), None
    )

    if not token:
        raise AccessToken_Error(
            res=res, message=f"Access token with ID {access_token_id} not found."
        )
...
async def generate_access_token(
    ...
    if not res.is_success or not res.response["token"]:
        raise AccessToken_GenerationError(user_id=user_id, res=res)

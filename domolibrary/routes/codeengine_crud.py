...
# %% auto 0
__all__ = ['CodeEnginePackageBuilder', 'deploy_code_engine_package', 'CodeEngine_InvalidPackage', 'create_code_engine_package',
           'increment_version', 'upsert_code_engine_package_version', 'upsert_package']
        ...
        if await codeengine_routes.test_package_is_released(
            ...
            if not auto_increment_version:
                raise CodeEngine_InvalidPackage(
                    message=f"Package {package_id} v{version} already deployed", auth=auth
                )

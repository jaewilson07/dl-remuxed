...
async def test_cell_3(token_auth=token_auth):
    ...
    content = {
        "update": "update_via_content",
        "id": 12387,
        "update_dt": str(dt.date.today()),
    }
...
async def test_cell_7(token_auth=token_auth):
    ...
    await AppDbDocument.get_by_id(
        auth=auth, debug_api=False, collection_id=COLLECTION_ID, document_id=DOCUMENT_ID
    )
...
async def test_cell_9(token_auth=token_auth):
    ...
    await TestDoc.get_by_id(
        collection_id=COLLECTION_ID, document_id=DOCUMENT_ID, auth=auth
    )

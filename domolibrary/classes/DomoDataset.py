...
# %% auto 0
__all__ = ['DomoDataset_Default', 'FederatedDomoDataset', 'DomoPublishDataset', 'DomoDataset']
...
from domolibrary.client.DomoEntity import DomoEntity_w_Lineage, DomoFederatedEntity, DomoPublishedEntity
    ...
    def _from_dict(
        ...
        **kwargs
        ...
        if not is_use_default_dataset_class:
            if not new_cls:
                raise NotImplementedError("Must provide new_cls if not using default dataset class")
            cls = new_cls
...
def _is_federated_dataset_obj(obj: dict) -> bool:
    ...
    has_federate = any(
        ["FEDERAT" in dpt, "FEDERAT" in disp]
        )
...
class FederatedDomoDataset(DomoDataset_Default,DomoFederatedEntity ):
    ...
    async def get_federated_parent(self, parent_auth : dmda.DomoAuth = None, parent_auth_retrieval_fn : Callable = None):   

...
class DomoPage(DomoEntity_w_Lineage):
    ...
    async def get_parents(self, page=None):
        page = page or self
    ...
    async def get_accesslist(
        ...
        for domo_user in domo_users:
            ...
            domo_user.custom_attributes["group_membership"] = [
                ...
                if int(domo_user.id)
                in [int(obj["id"]) for obj in group_obj.get("users")]
            ...
            match_group = next(
                (
                    ...
                    in [
                        int(domo_group.id)
                        for domo_group in domo_user.custom_attributes[
                            "group_membership"
                        ]
                    ]
    ...
    async def get_datasets(
        ...
        card_datasets = await dmce.gather_with_concurrency(
            *[
                card.get_datasets(debug_api=debug_api, session=session)
                for card in cards
            ],
            n=10,
        )
...
# Add methods to DomoPage class
DomoPage.get_children = get_children
DomoPage.flatten_children = flatten_children
DomoPage.get_parents = get_parents

# AUTOGENERATED! DO NOT EDIT! File to edit: ../../nbs/routes/instance_config_scheduler_policies.ipynb.

# %% auto 0
__all__ = ['Scheduler_Policies_Error', 'get_scheduler_policies', 'get_scheduler_policy_by_id', 'create_scheduler_policy',
           'update_scheduler_policy', 'delete_policy']

# %% ../../nbs/routes/instance_config_scheduler_policies.ipynb 3
...
# %% ../../nbs/routes/instance_config_scheduler_policies.ipynb 8
...
# %% ../../nbs/routes/instance_config_scheduler_policies.ipynb 10
@gd.route_function
async def get_scheduler_policies(
    ...
    if res.status == 403 and res.response.startswith('Forbidden'):
        raise Scheduler_Policies_Error(res=res, message="error retrieving permissions, is the feature switch enabled?")  
...
async def get_scheduler_policy_by_id(
    ...
    match_policy = next(( policy for policy in res.response if policy["id"] == policy_id), None)

    if not match_policy:    
        raise Scheduler_Policies_Error(res=rgd.ResponseGetData(
            status=404,
...
# %% ../../nbs/routes/instance_config_scheduler_policies.ipynb 14
@gd.route_function
async def create_scheduler_policy(
    ...
    if not isinstance(create_body["members"], list) or not all(
        isinstance(m, dict) and "type" in m and "id" in m for m in create_body["members"]
    ):
        raise Scheduler_Policies_Error(
            res=rgd.ResponseGetData(
                status=400,
...
# %% ../../nbs/routes/instance_config_scheduler_policies.ipynb 18
@gd.route_function
...
# %% ../../nbs/routes/instance_config_scheduler_policies.ipynb 22
@gd.route_function

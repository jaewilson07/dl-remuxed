...
if __name__ == "__main__":
    """
    Run route tests directly with various options.

    Usage:
        python tests/routes/test.py                    # Run all tests (imports + route tests)
        python tests/routes/test.py --imports-only     # Run only import tests
        python tests/routes/test.py --tests-only       # Run only route test files
        python tests/routes/test.py --list            # List all discovered test files
        python tests/routes/test.py --run user        # Run tests matching pattern 'user'

    Or with pytest:
        pytest tests/routes/test.py -v
        pytest tests/routes/test.py::TestRouteImports::test_all_route_files_can_be_imported -v
        pytest tests/routes/test.py::TestRouteTestExecution::test_run_all_route_tests -v
    """
    import argparse

    parser = argparse.ArgumentParser(description="Route Testing Utility")
    parser.add_argument(
        "--imports-only", action="store_true", help="Run only import tests"
    )
    parser.add_argument(
        "--tests-only", action="store_true", help="Run only route test files"
    )
    parser.add_argument(
        "--list", action="store_true", help="List all discovered test files"
    )
    parser.add_argument(
        "--run",
        type=str,
    ...
    if args.list:
        ...
        # Run only route test files
        tester = RouteImportTester()
        results = tester.run_all_test_files(verbose=args.verbose)
        success = results["failed_files"] == 0

    elif args.run:
        # Run specific test pattern
        success = run_specific_route_test(args.run)

    else:
        # Run all tests (default behavior)
        success = run_all_route_tests_standalone()
